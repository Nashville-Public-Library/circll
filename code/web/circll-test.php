...
function findNextMNPSOpenDate($dates, $searchDate) {
	...
	$searchDateObject = DateTime::createFromFormat('m/d/Y', $searchDate);

	// Check if the search date is in the list
	foreach ($dateObjects as $dateObject) {
		if ($dateObject == $searchDateObject) {
			...
			return $searchDateObject->format('m/d/Y');
		}
	}

	// If the search date is not in the list, return null
	return $searchDate;
}
